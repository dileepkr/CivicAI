...
const EmailGenerator: React.FC<EmailGeneratorProps> = ({ selectedPolicy, debateMessages }) => {
  ...
  const generateEmailContent = () => {
    ...
    const renterPoints = debateMessages
      .filter(msg => msg.sender === 'renter')
      .map(msg => msg.content)
      .join(' ');
    
    const landlordPoints = debateMessages
      .filter(msg => msg.sender === 'landlord')
      .map(msg => msg.content)
      .join(' ');
    ...
    // Get policy clauses that were discussed
    const discussedClauses = debateMessages
      .filter(msg => msg.clauseIndex !== undefined)
      .map(msg => msg.clauseIndex)
...
${discussedClauses.length > 0 ? `SPECIFIC POLICY SECTIONS DISCUSSED:
The following sections were part of our detailed discussion:
${discussedClauses.map(index => `• Clause ${index + 1}: Key provisions and implications were thoroughly analyzed`).join('\n')}
...
` : 'After carefully reviewing this policy and participating in the public discussion, '}${renterPoints ? `RENTER ADVOCATE PERSPECTIVE:
The following concerns were raised regarding tenant protections:
${renterPoints.substring(0, 400)}${renterPoints.length > 400 ? '...' : ''}

` : ''}${landlordPoints ? `LANDLORD PERSPECTIVE:
Property management concerns highlighted include:
${landlordPoints.substring(0, 400)}${landlordPoints.length > 400 ? '...' : ''}

` : ''}${moderatorSummary ? `BALANCED ANALYSIS:
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                <Button 
                  ...
                  disabled={!selectedPolicy || debateMessages.length === 0}
                >
              ...
              <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded">
                ...
                <p>Fill in all fields to enable sending. The email will open in your default email client with all fields pre-filled.</p>
              </div>
            </CardContent>
          </Card>
        )}
